...
if __name__ == '__main__':
    if umap_dim == 3:
        fig, ax = plt.subplots( ceil(len(chan_dic.keys())/4), 4, sharey=False, subplot_kw=dict(projection="3d"), figsize=(3*len(chan_dic.keys()), 3))
    elif umap_dim == 2:
        fig, ax = plt.subplots( ceil(len(chan_dic.keys())/4), 4, sharey=False, figsize=(8, 4))
    ...
    for i, channel in enumerate(chan_dic.keys()):
        ...
        ax[i//4][i%4].set_title(channel)
        if umap_dim==2:
            ax[i//4][i%4].scatter(embedding[:,0], embedding[:,1], c='maroon', marker='.', s=MARKER_SIZE)
        else:
            ax[i//4][i%4].scatter(embedding[:,0], embedding[:,1], embedding[:,2], marker='.', s=MARKER_SIZE, c='maroon')
    ...
    plt.savefig(f'{features}_features_umap_sub50-100.pdf')
    ...
    #         ax[j].set_yticks([])
    #     fig.tight_layout(pad=0.5)
    #     plt.savefig(f'imgs/{features}_{channel}_features_umap_sub0-50.pdf')
