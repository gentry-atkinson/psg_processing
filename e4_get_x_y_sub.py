...
def get_TWristAR():
    ...
    if (os.path.exists(zip_ffname)):
        ...
        # !wget fails if exported and run as .py
    ...
    if (os.path.isdir(os.path.join(my_dir, 'TWristAR'))):
        print("Skipping Unzip - Found existing dataset")
        return
    else:
        print("Unzipping TWristAR file in local directory")
        if (os.path.exists(zip_ffname)):
...
def unzip_e4_file(zip_ffname, my_dir):
...
def get_X_y_sub(
    working_dir= os.path.join(my_dir,'temp'), # this directory will be created
    zip_flist = [],
    time_steps = 96,
    step = 32 #if equal to time_steps there will be no overlap of sliding window
    ):
    """downloads TWristAR from Zenodo repository,
     processes each e4 zip file and associated label csv file into X (data),
...
interactive = False # just a way to jump between called and interactive work
